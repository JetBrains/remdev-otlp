...
class FromBackendOtlpConfigFactory(
    ...
    suspend fun createConfig(timeoutSeconds: Long = 10): OtlpConfig {
        try {
            ...
            return OtlpConfig(
                ...
                configuredMetricAttributes = remoteConfig.configuredMetricAttributes
            )
        } catch (e: Exception) {
    ...
    private fun updateLocalSettings(remoteConfig: OtlpRemoteConfig): OtpDiagnosticSettings {
        ...
        settings.updateBackendFrequentPerformanceMetricsReportingOverride(
            remoteConfig.frequentPerformanceMetricsReportingOverride
        )
        return settings
    }
